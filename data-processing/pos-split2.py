...
def split_train_test_fa(fasta_file):
	...
	for header in dict.keys():
		i += 1
		if i <= trainSplit:
			with open("pos-train.fa", "a") as ftrain:				
				ftrain.write(header) 
				ftrain.write('\n')
				ftrain.write(dict[header])
				ftrain.write('\n')
		elif i <= validSplit:
			with open("pos-valid.fa", "a") as ft:				
				ft.write(header) 
				ft.write('\n')
				ft.write(dict[header])
				ft.write('\n')
		elif i <= testSplit:
			with open("pos-test1.fa", "a") as ftest:
                                ftest.write(header)
                                ftest.write('\n')
                                ftest.write(dict[header])
                                ftest.write('\n')
		else:
			with open("pos-test2.fa", "a") as fvalid:				
				fvalid.write(header) 
				fvalid.write('\n')
...
split_train_test_fa("positive-cdhit.fa")
