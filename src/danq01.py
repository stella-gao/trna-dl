...
model_file_name = "./danq01.hdf5";
result_file_name = "./danq01.res";
...
#    _log.info('Building the model')
    ...
    model.add(Bidirectional(LSTM(input_dim=64, output_dim=64,
                                dropout_W=0.2, dropout_U=0.5,
                                # activation='relu',
                                return_sequences=True)))
...
fwr.close();
'''
fwr = open("danq01-fpr.txt", 'w')
fwr.write(str(false_positive_rate))
fwr.close();


fwr = open("danq01-tpr.txt", 'w')
fwr.write(str(true_positive_rate))
fwr.close();


fwr = open("danq01-precision.txt", 'w')
fwr.write(str(precision))
fwr.close();


fwr = open("danq01-recall.txt", 'w')
