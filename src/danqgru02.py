...
model_file_name = "./danqgru02.hdf5";
result_file_name = "./danqgru02.res";
...
#    model.add(MaxPooling1D(pool_length=2, stride=2))
    ...
    model.add(Bidirectional(GRU(input_dim=input_dim0, output_dim=64,
                                dropout_W=0.2, dropout_U=0.5,
                                 activation='relu',
                                return_sequences=True)))
...
fwr.close();
'''
fwr = open("danqgru02-fpr.txt", 'w')
fwr.write(str(false_positive_rate))
fwr.close();


fwr = open("danqgru02-tpr.txt", 'w')
fwr.write(str(true_positive_rate))
fwr.close();


fwr = open("danqgru02-precision.txt", 'w')
fwr.write(str(precision))
fwr.close();


fwr = open("danqgru02-recall.txt", 'w')
