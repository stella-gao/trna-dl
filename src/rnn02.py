...
model_file_name = "./rnn02.hdf5";
result_file_name = "./rnn02.res";
...
model.add(LSTM(32, input_shape=(134, 4), return_sequences=True))
...
fwr.close();
'''
fwr = open("rnn02-fpr.txt", 'w')
fwr.write(str(false_positive_rate))
fwr.close();


fwr = open("rnn02-tpr.txt", 'w')
fwr.write(str(true_positive_rate))
fwr.close();


fwr = open("rnn02-precision.txt", 'w')
fwr.write(str(precision))
fwr.close();


fwr = open("rnn02-recall.txt", 'w')
